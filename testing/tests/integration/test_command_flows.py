...
class TestCommandFlows:
    ...
    async def test_start_session_command_flow(self, async_emsp_client, async_mock_cpo_client,
        ...
        if command_result["result"] == "ACCEPTED":
            ...
            sessions_response = await async_mock_cpo_client.get(
                "/ocpi/cpo/2.2.1/sessions",
                headers=cpo_auth_headers
            )
            assert sessions_response.status_code == 200
    
    async def test_stop_session_command_flow(self, async_emsp_client, async_mock_cpo_client,
                                           emsp_auth_headers, cpo_auth_headers, test_data_factory):
        """Test complete stop session command flow."""
        # Step 1: Create an active session first
        active_session = test_data_factory.create_session(
            id="ACTIVE_SES_001",
            status="ACTIVE",
            party_id="CPO"
        )
        ...
        if command_result["result"] == "ACCEPTED":
            ...
            session_response = await async_mock_cpo_client.get(
                f"/ocpi/cpo/2.2.1/sessions/US/CPO/ACTIVE_SES_001",
                headers=cpo_auth_headers
            )
    ...
    async def test_reserve_now_command_flow(self, async_emsp_client, async_mock_cpo_client,
                                          emsp_auth_headers, cpo_auth_headers, test_data_factory):
        ...
        reserve_command = test_data_factory.create_command(
            command_type="RESERVE_NOW",
            response_url="https://emsp.example.com/ocpi/emsp/2.2.1/commands/RESERVE_NOW/789"
        )
        
        command_response = await async_emsp_client.post(
            "/ocpi/emsp/2.2.1/commands/RESERVE_NOW",
            headers=emsp_auth_headers,
            json=reserve_command
        )
        assert command_response.status_code in [200, 201]
        ...
        if command_result["result"] == "ACCEPTED":
            ...
            location_response = await async_mock_cpo_client.get(
                "/ocpi/cpo/2.2.1/locations",
                headers=cpo_auth_headers
            )
            assert location_response.status_code == 200
    
    async def test_cancel_reservation_command_flow(self, async_emsp_client, async_mock_cpo_client,
                                                 emsp_auth_headers, cpo_auth_headers, test_data_factory):
        ...
        command_response = await async_emsp_client.post(
            "/ocpi/emsp/2.2.1/commands/CANCEL_RESERVATION",
            headers=emsp_auth_headers,
            json=cancel_command
        )
        assert command_response.status_code in [200, 201]
    ...
    async def test_unlock_connector_command_flow(self, async_emsp_client, async_mock_cpo_client,
                                               emsp_auth_headers, cpo_auth_headers, test_data_factory):
    ...
    async def test_command_result_callback(self, async_emsp_client, emsp_auth_headers, test_data_factory):
        ...
        callback_result = test_data_factory.create_command_response(
            result="ACCEPTED"
        )
        
        # In real scenario, this would be called by CPO
        callback_response = await async_emsp_client.post(
            "/ocpi/emsp/2.2.1/commands/START_SESSION/callback",
            headers=emsp_auth_headers,
    ...
    async def test_concurrent_commands(self, async_emsp_client, emsp_auth_headers, test_data_factory):
        ...
        async def send_command(command_type, index):
            ...
            response = await async_emsp_client.post(
                f"/ocpi/emsp/2.2.1/commands/{command_type}",
                headers=emsp_auth_headers,
                json=command
            )
        ...
        tasks = [
            send_command("START_SESSION", i) for i in range(3)
        ] + [
