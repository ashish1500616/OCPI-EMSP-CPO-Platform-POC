...
class TestDataSynchronization:
    ...
    async def test_location_discovery_flow(self, async_emsp_client, async_mock_cpo_client,
                                         emsp_auth_headers, cpo_auth_headers, test_data_factory):
        """Test complete location discovery flow from CPO to EMSP."""
        # Step 1: CPO creates a new location
        new_location = test_data_factory.create_location(
            id="TEST_LOC_001",
        ...
        cpo_location_response = await async_mock_cpo_client.put(
            f"/ocpi/cpo/2.2.1/locations/US/CPO/TEST_LOC_001",
        ...
        emsp_location_response = await async_emsp_client.get(
    ...
    async def test_session_reporting_flow(self, async_emsp_client, async_mock_cpo_client,
                                        emsp_auth_headers, cpo_auth_headers, test_data_factory):
        ...
        cpo_session_response = await async_mock_cpo_client.put(
            f"/ocpi/cpo/2.2.1/sessions/US/CPO/TEST_SES_001",
            headers=cpo_auth_headers,
            json=new_session
        )
        ...
        emsp_sessions_response = await async_emsp_client.get(
    ...
    async def test_cdr_submission_flow(self, async_emsp_client, async_mock_cpo_client,
                                     emsp_auth_headers, cpo_auth_headers, test_data_factory):
        """Test CDR submission from CPO to EMSP."""
        # Step 1: CPO creates a CDR for completed session
        new_cdr = test_data_factory.create_cdr(
            id="TEST_CDR_001",
            party_id="CPO",
            session_id="SES001"
        )
        ...
        emsp_cdrs_response = await async_emsp_client.get(
    ...
    async def test_tariff_distribution_flow(self, async_emsp_client, async_mock_cpo_client,
                                          emsp_auth_headers, cpo_auth_headers, test_data_factory):
        """Test tariff distribution from CPO to EMSP."""
        # Step 1: CPO creates a tariff
        new_tariff = test_data_factory.create_tariff(
            id="TEST_TRF_001",
            party_id="CPO",
            currency="USD"
        )
        ...
        cpo_tariff_response = await async_mock_cpo_client.put(
            f"/ocpi/cpo/2.2.1/tariffs/US/CPO/TEST_TRF_001",
            headers=cpo_auth_headers,
            json=new_tariff
        )
        ...
        emsp_tariffs_response = await async_emsp_client.get(
    ...
    async def test_token_authorization_flow(self, async_emsp_client, async_mock_cpo_client,
                                          emsp_auth_headers, cpo_auth_headers, test_data_factory):
        """Test token authorization flow between EMSP and CPO."""
        # Step 1: EMSP provides token information to CPO
        test_token = test_data_factory.create_token(
            uid="TEST_TOKEN_001",
            party_id="EMS",
        ...
        emsp_token_response = await async_emsp_client.put(
            f"/ocpi/emsp/2.2.1/tokens/US/EMS/TEST_TOKEN_001",
            headers=emsp_auth_headers,
            json=test_token
        )
        ...
        cpo_auth_response = await async_mock_cpo_client.post(
            "/ocpi/cpo/2.2.1/tokens/US/EMS/TEST_TOKEN_001/authorize",
            headers=cpo_auth_headers,
            json=auth_request
        )
    ...
    async def test_data_pagination(self, async_emsp_client, emsp_auth_headers):
        ...
        response = await async_emsp_client.get(
            "/ocpi/emsp/2.2.1/locations?limit=2",
            headers=emsp_auth_headers
        )
    ...
    async def test_data_filtering(self, async_emsp_client, emsp_auth_headers):
        ...
        response = await async_emsp_client.get(
            f"/ocpi/emsp/2.2.1/sessions?date_from={date_from}&date_to={date_to}",
            headers=emsp_auth_headers
        )
        assert response.status_code == 200
    ...
    @pytest.mark.slow
    async def test_large_dataset_synchronization(self, async_emsp_client, async_mock_cpo_client,
                                                emsp_auth_headers, cpo_auth_headers, test_data_factory):
