...
class OCPIEducationalDemo:
    ...
    async def demo_authentication(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            self.print_step(1, "Version Discovery", 
                          "EMSP discovers what OCPI versions the CPO supports")
            
            try:
                response = await client.get(
                    f"{self.emsp_base_url}/ocpi/emsp/2.2.1/versions",
                    headers=self.emsp_headers
                )
            ...
            self.print_step(2, "Endpoint Discovery",
                          "Discover available OCPI modules and their endpoints")
            
            try:
                response = await client.get(
                    f"{self.emsp_base_url}/ocpi/emsp/2.2.1/versions/2.2.1",
                    headers=self.emsp_headers
                )
            ...
            self.print_step(3, "Credential Exchange",
                          "Exchange credentials to establish secure communication")
    ...
    async def demo_location_discovery(self):
        ...
        self.print_section_header(
            ...
            "Learn how EMSPs discover charging stations from CPOs"
        )
        ...
        async with httpx.AsyncClient() as client:
            ...
            self.print_step(1, "Location Discovery",
                          "EMSP requests list of available charging locations from CPO")
            
            try:
                response = await client.get(
                    f"{self.emsp_base_url}/ocpi/emsp/2.2.1/locations",
                    headers=self.emsp_headers
                )
    ...
    async def demo_token_authorization(self):
        ...
        self.print_section_header(
            ...
            "Learn how EMSP users get authorized to charge at CPO stations"
        )
        ...
        self.print_step(1, "Authorization Request",
                      "CPO requests authorization for user token")
    ...
    async def demo_session_management(self):
        ...
        self.print_section_header(
            ...
            "Learn how charging sessions are started, monitored, and stopped"
        )
        ...
        self.print_step(1, "Start Session Command",
                      "EMSP sends command to CPO to start charging session")
        ...
        self.print_step(2, "Session Monitoring",
                      "Monitor active charging session progress")
        ...
        # Step 3: Stop Session
        self.print_step(3, "Stop Session Command",
                      "User or system initiates session termination")
    ...
    async def demo_cdr_processing(self):
        ...
        self.print_section_header(
            ...
            "Learn how billing information is exchanged after charging sessions"
        )
        ...
        self.print_step(1, "CDR Generation",
                      "CPO generates detailed billing record after session completion")
            ...
            "charging_periods": [
                {
                    "start_date_time": "2024-01-15T10:30:00Z",
                    "dimensions": [
                        {"type": "ENERGY", "volume": 18.2},
                        {"type": "TIME", "volume": 52.0}
                    ]
                }
            ]
        }
        ...
        self.print_step(2, "Billing Integration",
                      "EMSP processes CDR for customer billing")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="OCPI Educational Demo Runner",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__
    )
    
    parser.add_argument("demo_type", nargs="?", default="interactive",
                       choices=["all", "authentication", "locations", "tokens", 
                               "sessions", "cdrs", "interactive"],
