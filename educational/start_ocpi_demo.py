...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('ocpi_demo.log')
    ]
)
    ...
    def start_emsp_backend(self):
        ...
        try:
            ...
            if os.path.exists("Pipfile"):
                cmd = ["pipenv", "run", "python", "-m", "uvicorn", "main:app", 
                       "--host", "0.0.0.0", "--port", str(self.emsp_port)]
            else:
            ...
            if self.emsp_process.poll() is None:
                print(f"✅ EMSP Backend started successfully!")
...
"""
            ...
            if self.cpo_process.poll() is None:
                print(f"✅ Mock CPO Server started successfully!")
    ...
    async def start_demo(self, start_emsp: bool = True, start_cpo: bool = True, 
                        run_tests: bool = True):
...
def main():
    ...
    parser.add_argument("--emsp-only", action="store_true", 
                       help="Start only EMSP backend")
    parser.add_argument("--cpo-only", action="store_true", 
                       help="Start only Mock CPO server")
    parser.add_argument("--no-tests", action="store_true", 
                       help="Skip running demo tests")
    parser.add_argument("--port-emsp", type=int, default=8000, 
                       help="EMSP port (default: 8000)")
    parser.add_argument("--port-cpo", type=int, default=8001, 
                       help="Mock CPO port (default: 8001)")
